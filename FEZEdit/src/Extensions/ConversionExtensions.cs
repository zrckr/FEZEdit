...
internal static class ConversionExtensions
{
    ...
    public static Mesh ToGodotMesh(this FEZRepacker.Core.Definitions.Game.XNA.Texture2D texture2D)
    {
        var size = new Vector2(texture2D.Width, texture2D.Height);
        var material = new StandardMaterial3D
        {
            AlbedoTexture = texture2D.ToImageTexture(),
    ...
    public static Mesh ToGodotMesh(this FEZRepacker.Core.Definitions.Game.Graphics.AnimatedTexture animatedTexture)
    {
        ...
        var image = animatedTexture.ToImageTexture().GetImage().GetRegion(region);
        var material = new StandardMaterial3D
        {
            AlbedoTexture = new AtlasTexture { Atlas = ImageTexture.CreateFromImage(image), Region = region },
            TextureFilter = BaseMaterial3D.TextureFilterEnum.Nearest
        };
